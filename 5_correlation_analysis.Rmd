...
output_date <- "2022-11-24"
ms_version <- "MS_v8" #Version of MS outputs are for
section <- "correlations"
input_date <- "2022-03-18" # Date of clean data production we want to use

analysis_date <- "2022-03-18"
...
ecoregion_geometry <- ecoregion_map_all %>%  
...
quantile_sf <- indicators_raw_no_lpi %>% 
  ...
  merge(ecoregion_geometry[c("ECO_NAME","geometry")],
                           by = "ECO_NAME") %>% 
                     group_by(cluster) %>%
     mutate(rli_quantile = ntile(RLI, 4),
            bii_quantile = ntile(BIIri, 4)) %>% 
   mutate(group = paste(rli_quantile, bii_quantile),
          cluster_n = n()) %>% 
   ungroup(.) %>% 
...
quantile_plot <- ggplot(data = quantile_df,
               aes(x = bii_quantile,
                   y = rli_quantile)) +
         geom_tile(aes(x = bii_quantile,
                      y = rli_quantile,
                      fill = prop),
                    alpha = 0.8) +
        ...
        labs(x = "Biodiversity Intactness Index (richness) quantiles",
             y = "Red List Index quantiles")
...
st_write(quantile_sf, file.path(clean_data, "9_indicator_quartile_rankings.shp"))
