...
make_heatmap <- function(data, directory, name) {
  ...
  ggsave(file.path(directory, name, ".png", sep = ""),
         heatmap, device = "png")
...
#saveRDS(pairwise_correlations, file.path(supp_outputs, "correlation_dataframe.rds"))
# write.csv(pairwise_correlations, file.path(supp_outputs, "correlation_dataframe.csv"))
...
```{r heatmaps, echo=FALSE, warning = FALSE, message = FALSE}

global_heatmap_no_lpi_data <- pairwise_correlations_no_lpi %>% 
                              filter(subgroup == "all.biome") %>% 
                              arrange(coefficient)

global_heatmap_no_lpi <- make_heatmap(global_heatmap_no_lpi_data,
                                      main_outputs,
                                      "global_heatmap_no_lpi")


global_heatmap_no_lpi

global_heatmap_lpi_data <- pairwise_correlations_lpi %>% 
                              filter(subgroup == "all.cluster") %>% 
                              arrange(coefficient)

global_heatmap_lpi <- make_heatmap(global_heatmap_lpi_data,
                                   main_outputs,
                                   "global_heatmap_no_lpi")
