...
# Australia comparison v1 ----

## Government data (source?)

gov_species_data_all <- read.csv("N:/Quantitative-Ecology/Simone/extinction_test/inputs/australian_threatened_species_gov/2020_threatened_species_government_list.csv")

head(gov_species_data_all)

# Get synonyms ----

aus_binomials <- unique(gov_species_data_all$Scientific.Name)

aus_synonyms <- find_synonyms(aus_binomials)

gov_species_data <- gov_species_data_all %>%
                    merge(aus_synonyms[c("binomial", "tsn")],
                          by.x = "Scientific.Name",
                          by.y = "binomial") %>%
                    filter(Class == c("Mammalia", "Aves", "Amphibia")) %>%
                    select(Scientific.Name, tsn, Threatened.status) %>%
                    distinct(.)

head(gov_species_data)
head(species_data)

# Number of unique species on Government list

length(unique(gov_species_data$tsn))

# Number of unique species on our list

length(unique(species_data$tsn))

species_data_comp <- species_data %>%
                     group_by(tsn) %>%
                     ...
                     distinct(.) %>%
                     merge(gov_species_data, by = "tsn", all = TRUE) %>%
                     mutate(aus_status = ifelse(Threatened.status == 
                                                       "Vulnerable", "VU",
                                              ifelse(Threatened.status == 
                                                       "Endangered","EN",
                                              ifelse(Threatened.status == 
                                              ...
                                              ifelse(Threatened.status ==
                                                     ...
                                                     NA))))))) %>%
                          select(tsn, binomial,redlist_status, aus_status,
                                 Scientific.Name,
                                 Threatened.status) %>%
                          mutate(match_status = 
                                   ifelse(redlist_status == aus_status,
                                          TRUE, FALSE)) 

head(species_data_comp)

table(species_data_comp$match_status)

spp_on_our_list_only <- species_data_comp %>%
                        filter(is.na(aus_status))

spp_on_gov_list_only <- species_data_comp %>%
                        filter(is.na(redlist_status))

table(spp_on_our_list_only$redlist_status)

table(spp_on_gov_list_only$aus_status)

# ~ 163 with threatened status, 1700 least concern, 84 NT, 24 DD
# which make sense because australian list doesn't include not 
# threatened/least concern. However, still around 100 more spp on our list.
# Have looked at quite a few, all the ones I looked at are from Australia

# Australia comparison v2  ----
