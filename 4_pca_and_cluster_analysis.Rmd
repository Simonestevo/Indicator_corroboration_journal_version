...
output_date <- "2022-11-24" # Current date for outputs
...
df <- cluster_biplot_data %>% 
      dplyr::select(-LPI_2005, - geometry, -ECO_NAME, -OBJECTID) %>% 
...
TrainingData <- df[TrainingRowIndex, ]  # Take 80% of the data to fit the models
TestData  <- df[-TrainingRowIndex, ]   # Keep 20% to test how accurate the selected model is


mod <- rpart(cluster ~ disturbance_year + high_beta_area_factor +
               island_status + predominant_threat_type +
               lpi_records_factor + rli_records_factor, data = TrainingData, 
...
modp <- rpart.plot(mod,type = 3, fallen.leaves = TRUE)
modp

# Make predictions on the test dataset
TestData$pred <- predict(mod, TestData, type = "class")

# Examine the confusion matrix
table(TestData$pred , TestData$cluster )

# Compute the accuracy on the test dataset
mean(TestData$pred == TestData$cluster)

# Examine the complexity plot
plotcp(mod)

# Prune the tree
mod_pruned <- prune(mod, cp = 0.093)
plotcp(mod_pruned)

# Compute the accuracy of the pruned tree
TestData$pred <- predict(mod_pruned, TestData, type = "class")
mean(TestData$pred == TestData$cluster)
