...
```{r pca, echo=FALSE, warning = FALSE, message = FALSE, fig.cap = "biplot TBD"}
...
fviz_eig(res.pca24, addlabels = TRUE, ylim = c(0, 50))

dev.off()

# Alt - Using princomp function 
...
test <- prcomp(pc.f2, cor=TRUE, data=pl2.data)
summary(test)
...
test.out <- hclust(dist(test$x[, 1:4]), method = "complete")

wisc.pr.hclust.clusters <- cutree(test.out, k = 3)
...
# text(pl2.pca$scores, labels = as.character(row.names(pl2.data)), pos=1, cex=0.7, add = TRUE)
...
res.hcpc2 <- HCPC(res.pca24,nb.clust = -1, graph = FALSE)

# Tried including numerical variables inside PCA and cluster, but ends up with like 8 dimensions
# x <- indicators_std_no_lpi %>% 
#       merge(ecoregions_numeric_std, by = "ecoregion_id") %>% 
#       dplyr::select(-ecoregion_id)
# 
# testpca <- PCA(x, ncp = 5, graph = FALSE)
# testclust <- HCPC(testpca,nb.clust = -1, graph = FALSE)
...
test_data <- cluster_map_data[1:50,]


cluster_map <-  ggplot(test_data) +
...
pca_cluster_plot <- fviz_pca_biplot(res.pca24, geom.ind = "point", 
                                    pointshape = 21, 
                                    pointsize = 2, 
                                    #col.ind = "black", 
                                    palette = mag3,
