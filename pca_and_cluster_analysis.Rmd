...
res.desc <- dimdesc(res.pca24, axes = c(1,2), proba = 0.05)
...
res.hcpc2 <- HCPC(res.pca24,nb.clust = -1, graph = FALSE) # - 1 lets algorithm select number of clusters
...
cluster_map <-  ggplot(cluster_map_data) +
  geom_sf(aes(fill = cluster), colour = "black", 
          size = 0.05, show.legend = 'fill', alpha = 0.8) +
...
xy <- st_bbox(afrotropic)
asp2 <- (xy$xmax - xy$xmin)/(xy$ymax - xy$ymin)

insetmap = tm_shape(afrotropic) + tm_fill(col="lightgrey") +
  m_borders(lwd = 1, col="grey") +
