...
sig_threshold <- 0.01 # Make it 0.01 to correct for multiple comparisons
output_date <- "2022-11-24" # Current date for analysis outputs
ms_version <- "MS_v8" #Version of MS outputs are for
section <- "correlation"
input_date <- "2022-11-24" # Date of indicator data output we are using


analysis_date <- "2022-11-24"
...
parent_outputs <- "N:/LES/Burwood/Quantitative-Ecology/Indicators-Project/ecoregions/"
...
manuscript_outputs <- "N:/LES/Burwood/Quantitative-Ecology/Indicators-Project/ecoregions/2021-05-04_manuscript_outputs"

# Folder for clean data that acts as inputs to all subsequent steps

clean_data <- file.path(manuscript_outputs, output_date, paste(ms_version,
                                 "_cleaned_data",sep=""))
...
# Shorten the biome names

ecoregions_categorical <- ecoregions_categorical %>% 
                          mutate(biome = abbreviate(biome, 15),
                                 predominant_threat_type = abbreviate(predominant_threat_type, 15))
...
if(load_map == TRUE) {
  ...
  ecoregion_map_all <- readRDS(paste(
    file.path("N:/LES/Burwood/Quantitative-Ecology/Indicators-Project/ecoregions/inputs",
...
ecoregion_geometry <- ecoregion_map_renamed %>%  
                      dplyr::select(OBJECTID, ECO_NAME, geometry)

rm(ecoregion_map_all)

threat_scheme <- read.csv(file.path("N:/LES/Burwood/Quantitative-Ecology/Indicators-Project/ecoregions/inputs/iucn_threats/iucn_threat_classification_scheme.csv"))
  ...
  for (j in seq_along(group_indicator_list)) {
    ...
    if (nrow(group_matrix) < 3) {
      ...
      rs_ci <- cor.ci(group_matrix, n.iter = 2000, # n iterations taken from Ruscio et al 2008
                      p = sig_threshold, # 0.01 not 0.05 bc correcting for multiple comparisons as in Sievers et al
      ...
      p.mat <- cor_pmat(group_matrix, sig.level = sig_threshold)
...
pairwise_correlations_no_lpi <- pairwise_correlations_cf %>% 
  ...
  mutate(signif_2 = ifelse((pvalue > sig_threshold | lower_ci < 0 & upper_ci > 0),
                           0, 1))
...
names(indicators_raw_no_lpi) <- c("ecoregion_id", "BHI_plants", "BIIab",
                                  "BIIri", "HFP", "extinct",
                                  "RLI", "threatened")
...
p_mat = cor_pmat(cor_inputs_no_lpi, sig.level = 0.01)
...
correlations_edited <- all_correlations %>% 
                       ...
                       #Switch non-significant(pval < threshold AND 99% CI do not cross 0) coefficients to 0
                       mutate(coefficient = ifelse(signif_2 == 1, coefficient, 0)) %>%
                       ...
                       mutate(coeff_label = ifelse(input_relationship == "convergent", # identify convergent comparisons with a *
                                                   paste(coeff_ch, " (c)", sep = "", collapse = NULL),
                                                   paste(coeff_ch, " (d)", sep = "")))
...
global_heatmap_no_lpi_1 <- ggcorrplot(cor_outputs_no_lpi,
                                    ...
                                    type = "lower",
                                    outline.color = "white",
                                    ggtheme = theme(panel.background = 
                                                    element_blank())) + 
                                    labs(fill='Corr. strength') 

global_heatmap_no_lpi_1

# ggsave(file.path(main_outputs, "global_heatmap_no_lpi_pretty.png"),
#          global_heatmap_no_lpi_1, device = "png")

## GGplot version - uglier but easier to edit

heatmap_pal_gradient <- scales::seq_gradient_pal("gray30", "white", "Lab")(seq(0,1,length.out=5))
...
global_heatmap_no_lpi <- ggplot(global_heatmap_no_lpi_data, aes(x = Ind1, 
  ...
  geom_text(aes(label = coeff_label), size = 7) +
...
tri_data <- flipped_correlations %>% 
      dplyr::select(Ind1, Ind2, coefficient, corr_level, coeff_label) %>% 
      rbind(self_corr) %>% 
      mutate(coefficient = ifelse(coefficient == 0.00, "", coefficient)) %>% 
                        mutate(Ind1 = factor(Ind1, 
                               levels = c("BHI_plants","BIIab", "BIIri", "HFP",
                                          "extinct", "RLI", "threatened")),
                               Ind2 = factor(Ind2, 
                               levels = c("BHI_plants","BIIab", "BIIri", "HFP",
                                          "extinct", "RLI", "threatened")))
 

pal2 <- c("#357BA2FF", "#49C1ADFF", "#DEF5E5FF", "white","grey80")

global_heatmap_no_lpi_triangle <- ggplot(tri_data, aes(x = Ind1, y = Ind2, 
                                fill = corr_level)) +
  geom_tile(color = "white", height = 1.0, alpha = 0.75) +
  geom_text(aes(label = coefficient), size = 7) +
...
ggsave(file.path(main_outputs, "global_heatmap_no_lpi_triangle.png"),
       global_heatmap_no_lpi_triangle, width = 10, height = 10, units = "in",
       device = "png")
```
...
names(indicators_raw_lpi) <- c("ecoregion_id", "BHI_plants", "BIIab",
                                  "BIIri", "HFP", "extinct", "LPI",
                                  "RLI", "threatened")
# Pretty heatmap
...
p_mat <- cor_pmat(cor_inputs_lpi, sig.level = sig_threshold)

global_heatmap_lpi <- ggcorrplot(cor_outputs_lpi,
                                  colors = c("#481568FF","white", "#20A386FF"),
                                  sig.level = sig_threshold,
                                  #insig = "blank",
                                    #hc.order = TRUE,
                                    p.mat = p_mat,
                                    lab = TRUE,
...
ggsave(file.path(main_outputs, "global_heatmap_lpi_pretty.png"),
         global_heatmap_lpi, device = "png")

## Only ecoregions with > 20 pops

cor_inputs_lpi_20 <- indicators_raw_lpi %>%
                  merge(ecoregions_categorical[c("ecoregion_id",
                                                 "lpi_records_factor")]) %>% 
              dplyr::select(-ecoregion_id) %>%
              filter(lpi_records_factor == "more than 20 lpi pops") %>% 
              dplyr::select(-lpi_records_factor) %>%
              as.matrix(.)

cor_outputs_lpi_20 <- cor(cor_inputs_lpi_20, method = "spearman")
p_mat <- cor_pmat(cor_inputs_lpi_20, sig.level = sig_threshold)

global_heatmap_lpi_20 <- ggcorrplot(cor_outputs_lpi_20,
                                   colors = c("#481568FF","white", "#20A386FF"),
                                    #sig.level = 0.05,
                                    #insig = "blank",
                                    #hc.order = TRUE,
...
ggsave(file.path(main_outputs, "global_heatmap_lpi_20_pretty.png"),
         global_heatmap_lpi_20, device = "png")


# Ugly but more flexible heatmap
...
correlations_edited <- all_correlations %>% 
                       filter(subgroup == "all.cluster") %>% # Get just global coeffs
                         #Switch non-significant(pval < threshold AND 95% CI do not cross 0) coefficients to 0
                       mutate(coefficient = ifelse(signif_2 == 1, coefficient, 0)) %>%                        mutate(corr_level = ifelse(coefficient > -0.3 & # assign the rankings
                                                  coefficient < 0, "weak",
                                           ifelse(coefficient < 0.3 & # assign the rankings
                                                  coefficient > 0, "weak",
                                           ifelse(coefficient > 0.3 &
...
cor_inputs_no_lpi_clusters <- indicators_raw_no_lpi %>%
              merge(ecoregion_clusters[c("ecoregion_id", "cluster")]) %>% 
              filter(cluster != "Incomplete data") 

cor_inputs_cluster_list <- split(cor_inputs_no_lpi_clusters,
                                 cor_inputs_no_lpi_clusters$cluster)

pretty_cluster_heatmaps <- list()

for ( i in seq_along(cor_inputs_cluster_list)) {

  clus <- cor_inputs_cluster_list[[i]]$cluster[1] 
  
  clus
  
  inputs <- cor_inputs_cluster_list[[i]] %>% 
            dplyr::select(-ecoregion_id, - cluster) %>%
                    as.matrix(.)

cor_outputs <- cor(inputs, method = "spearman")

p_mat = cor_pmat(inputs, sig.level = sig_threshold)

if (clus == "1") {
  
pretty_cluster_heatmaps[[i]] <- ggcorrplot(cor_outputs,
                                    colors = c("#481568FF","white", "#20A386FF"),
                                    #sig.level = 0.05,
                                    #insig = "blank",
                                    #hc.order = TRUE,
...
ggsave(file.path(main_outputs, paste("cluster", clus, 
                                     "global_heatmap_lpi.png", sep = "_")),
       pretty_cluster_heatmaps[[i]], width = 10, height = 10, units = "in",
       device = "png")

}

cluster_heatmaps <- plot_grid(pretty_cluster_heatmaps[[1]],
                        pretty_cluster_heatmaps[[2]],
                        pretty_cluster_heatmaps[[3]],
                        nrow = 1, align = "v")

ggsave(file.path(main_outputs, "cluster_heatmaps_pretty.png"),
       cluster_heatmaps, width = 30, height = 12, units = "in",
       device = "png")
...
cluster_heatmap <- ggplot(cluster_heatmap_data, 
  ...
  coord_equal() +
  geom_text(aes(label = coeff_label), size = 4) +
  scale_fill_manual(values = heatmap_pal_vir) +
  labs(x = "Indicator 1",
       y = "Indicator 2") +
    guides(fill = guide_legend(title = "Correlation\nstrength")) +
...
ggsave(file.path(main_outputs, "cluster_heatmaps.png"),
       cluster_heatmap, width = 16, height = 8, units = "in",
       device = "png")


## Triangle heatmap

prep_heatmap_data_tri <- function(data, sub_name) {
...
clus1 <- prep_heatmap_data_tri(cluster_data[[1]], "Cluster one")
clus2 <- prep_heatmap_data_tri(cluster_data[[2]], "Cluster two")
clus3 <- prep_heatmap_data_tri(cluster_data[[3]], "Cluster three")
...
cluster_triangles <- ggplot(cluster_heatmap_data, aes(x = Ind1, y = Ind2, 
                                fill = corr_level)) +
  geom_tile(color = "white", height = 1.0, alpha = 0.75) +
  geom_text(aes(label = coefficient), size = 7) +
  scale_fill_manual(values = pal2) +
...
ggsave(file.path(main_outputs, "cluster_heatmap_triangle.png"),
       cluster_triangles, width = 20, height = 8, units = "in",
       device = "png")
...
rank_rli_bhi_scatterplot_data <- input_data %>% 
                            ...
                            mutate(cluster_label = factor(cluster_label, 
                               levels = c("Cluster one","Cluster two",
                                          "Cluster three"))) %>% 
                            mutate(BHI_rank_order = rank(BIIri, "last"),
                                   RLI_rank_order = rank(RLI))
...
```{r rank priority ecoregions, echo=FALSE, warning = FALSE, message = FALSE}

top <- -10

bii_priorities <- indicators_raw_no_lpi %>% 
                     merge(ecoregion_clusters[c("ecoregion_id", 
                                                "ECO_NAME",
                                                "cluster")],
                           by = "ecoregion_id") %>% 
                     group_by(cluster) %>% 
                     top_n(top, BIIri) %>% 
                     dplyr::select(ECO_NAME, cluster, BIIri) %>% 
                     ungroup(.) %>% 
                     arrange(cluster)
                      
kable(bii_priorities)

rli_priorities <- indicators_raw_no_lpi %>% 
                     merge(ecoregion_clusters[c("ecoregion_id", 
                                                "ECO_NAME",
                                                "cluster")],
                           by = "ecoregion_id") %>% 
                     group_by(cluster) %>% 
                     top_n(top, RLI) %>% 
                     dplyr::select(ECO_NAME, cluster, RLI) %>% 
                     ungroup(.) %>% 
                     arrange(cluster)
                      
kable(rli_priorities)
...
# This plot aims to show how many ecoregions are in different quantiles based
# on different indicators (e.g if an ecoregion is in the same quantile for both 
# indicators you might consider there to be consensus, but if they're at extreme
# ends of the quantile it indicates contradiction between indicator scores)

quantile_sf <- indicators_raw_no_lpi %>% 
  merge(ecoregion_clusters[c("ecoregion_id", "cluster", "ECO_NAME")],
                           by = "ecoregion_id") %>% 
  # add year of disturbance
  merge(ecoregions_categorical[c("ecoregion_id","disturbance_year")],
                           by = "ecoregion_id") %>% 
                     group_by(cluster) %>%
  dplyr::select(ecoregion_id, ECO_NAME, cluster, BHI_plants, RLI) %>% 
     mutate(spp_quantile = ntile(RLI, 4),
            lu_quantile = ntile(BHI_plants, 4)) %>% 
   mutate(group = paste(spp_quantile, lu_quantile),
          cluster_n = n()) %>% 
   ungroup(.) %>% 
   group_by(cluster, group) %>% 
   mutate(membership_n = n(),
          prop = membership_n/cluster_n) %>% 
   distinct(.) %>% 
   arrange(desc(cluster)) %>% 
   arrange(desc(prop)) %>% 
   ungroup(.) %>% 
   mutate(agreement_level = lu_quantile - spp_quantile) %>% 
  # Make sf so we can map it later
  merge(ecoregion_geometry,by = "ECO_NAME")

quantile_plot <- ggplot(data = quantile_sf,
               aes(x = lu_quantile,
                   y = spp_quantile)) +
         geom_tile(aes(x = lu_quantile,
                      y = spp_quantile,
                      fill = prop),
                    alpha = 0.8) +
        ...
        labs(x = "Land use quantiles",
             y = "Species quantiles")

quantile_plot

ggsave(file.path(supp_outputs, "quantile_plot.png"),
         quantile_plot, device = "png")

quantile_sf <- quantile_sf %>% 
               dplyr::select(ecoregion_id, ECO_NAME, cluster, spp_quantile, 
                             lu_quantile, RLI, BHI_plants, agreement_level, 
                             geometry) %>% 
               st_as_sf()

names(quantile_sf) <- abbreviate(names(quantile_sf), 8)

st_write(quantile_sf, 
         file.path(clean_data, "9_indicator_quantile_rankings.shp"))

disturbed_sf <- quantile_sf %>% 
                dplyr::select(ecorgn_d, ECO_NAME, cluster,geometry) %>% 
                merge(ecoregions_categorical[c("ecoregion_id", 
                                               "disturbance_year")], 
                      by.x = "ecorgn_d",
                      by.y = "ecoregion_id") %>% 
                mutate(disturbed = ifelse(disturbance_year == "undisturbed", 
                                          "undisturbed", "disturbed")) 

# Trying to make polygons of just diturbed/undisturbed locations
# center <- disturbed_sf %>% # select the central parts
#   filter(disturbed == "disturbed") %>% 
...
#   mutate(disturbed = "disturbed") # return back the data value 
# 
# outskirts <- disturbed_sf%>% # select the central parts
#   filter(disturbed == "undisturbed") %>% 
#   st_buffer(0.5) %>% # make a buffer of half a meter around all parts (to avoid slivers)
#   st_union() %>% # unite to a geometry object
#   st_sf() %>% # make the geometry a data frame object
#   mutate(disturbed = "undisturbed") # return back the data value 
# 
# frmPraha2 <- rbind(center, 
#                    outskirts)
# 
# plot(st_geometry(frmPraha2))
# 
# st_write(frmPraha2,file.path(clean_data, 
#                              "10_disturbed_ecoregions.shp"), append = FALSE)
