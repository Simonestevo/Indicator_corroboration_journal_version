---
title: "PCA and cluster analysis"
author: "Simone Stevenson"
date: "05/05/2021"
output:
    html_document: default
    pdf_document: default
objective: This code runs a PCA on the clean indicator data and does a cluster analysis
---


This code should be run after exploratory data analysis but before correlation analysis code.
...
analysis_inputs <- "N:/LES/Burwood/Quantitative-Ecology/Indicators-Project/ecoregions/outputs/version_3/2020-08-25_indicator_output_files"
save_outputs <- "yes" #only applies to maps, other things will always save
eco_version <- "ecoregions_2017"
parent_outputs <- "N:/LES/Burwood/Quantitative-Ecology/Indicators-Project/ecoregions/outputs"
...
manuscript_outputs <- "N:/LES/Burwood/Quantitative-Ecology/Indicators-Project/ecoregions/outputs/version_3/2021-05-04_manuscript_outputs"
...
```{r PCA w explanatory variables as well}

# # Scale the data to a mean of 0 and sd of 1
# mixData <- scale(pca_data_5[,2:8])
# 
# mixData <- cbind(ecoregion_id = pca_data_5[,1],mixData, pca_data_5[,10:ncol(pca_data_5)])
# 
# summary(mixData)
# 
# test <- FAMD(mixData,
#                   graph=TRUE)
# 
# fviz_famd_ind(test,col.ind = "cos2",
...
cluster_map_data <- ecoregion_map_renamed %>%
  ...
  mutate(cluster = ifelse(is.na(cluster), "Incomplete data", cluster))
...
clus1_names <- c("Afrotropic","Australasia", "Neotropic",
                 "Hawaii", "Northern Marianas", "Palearctic")

clus1_split <- list(clus1_split[[1]], clus1_split[[2]],
                    clus1_split[[3]],hawaii, marianas, clus1_split[[5]])
...
for (i in seq_along(clus1_split)) {
  ...
  x <- st_bbox(clus1_split[[i]])
  cropped_cluster <- st_crop(cluster_map_data, x)

  st_write(cropped_cluster,
           file.path(data_outputs, paste("10.", i, "_", 
                                         clus1_realm_name, sep = "")), 
           append = FALSE)
}
...
cluster_map <-  ggplot(cluster_map_data) +
  ...
  scale_fill_manual(values = c(plas3, "grey80"),
                    na.value = 'grey70') +
  labs(fill = "Cluster") +
  xlab("Longitude") + 
  ylab("Latitude") +
...
oceania <- plot_grid(cluster_1_maps[[2]],
          cluster_1_maps[[5]],
          nrow = 2, align = "v")
...
#       cluster_map + 
#       coord_sf(
#     xlim = sf::st_bbox(cluster_1_maps[[1]])[c(1,3)],
#     ylim = sf::st_bbox(cluster_1_maps[[1]])[c(2,4)],
#     expand = FALSE
#     ) +
...
#   draw_plot(cluster_1_maps[[1]], x = 0, y = -0.25, width = 0.4, height = 0.2)
# 
# test_map
...
write.csv(cluster_data, file.path(clean_data, "8_ecoregion_clusters.csv"))
saveRDS(cluster_data, file.path(clean_data, "8_ecoregion_clusters.rds"))
...
chi_results_df <- do.call(rbind, cluster_chisq_results)

write.csv(chi_results_df, file.path(supp_outputs, "chisq_results.csv"))
...
cluster_barplot_data <- cluster_barplot_data %>%
  ...
  summarise(ecoregion_count = dplyr::n_distinct(ecoregion_id)) %>%
  ungroup()
...
# Following datacamp tutorial https://campus.datacamp.com/courses/supervised-learning-in-r-classification/classification-trees-4?ex=2

library(rpart)
library(tidymodels)

df <- ecoregions_categorical %>% 
      merge(cluster_biplot_data[c("ecoregion_id", "cluster")],
            by = "ecoregion_id") %>% 
      mutate(biome = abbreviate(biome, minlength = 6),
             predominant_threat_type = abbreviate(predominant_threat_type, 
                                                  minlength = 6),
             cluster = as.factor(cluster),
             disturbance_yearCh = as.character(disturbance_year),
             disturbed = factor(ifelse(disturbance_yearCh == "undisturbed",
...
# Create the split
df_split <- initial_split(df, prop = 0.8, strata = cluster)

# Extract the training and test set
df_train <- training(df_split)
df_test  <- testing(df_split)

mod <- rpart(cluster ~ biome + disturbance_year + area_factor + 
             island_status + land_use +
             lpi_records_factor + rli_records_factor, data = df_train, 
...
rpart.plot(mod,type = 3, fallen.leaves = TRUE)

# Make predictions on the test dataset
df_test$pred <- predict(mod, df_test, type = "class")

# Examine the confusion matrix
table(df_test$pred , df_test$cluster)

# Compute the accuracy on the test dataset
mean(df_test$pred == df_test$cluster)
...
# Prune the tree
mod_pruned <- prune.rpart(mod,cp = 0.092)
plotcp(mod_pruned)
rpart.plot(mod_pruned,type = 5, fallen.leaves = TRUE)

# Compute the accuracy of the pruned tree
df_test$pred <- predict(mod_pruned, df_test, type = "class")
mean(df_test$pred == df_test$cluster)
...
# 
# # Create the split
# df_split <- initial_split(df, prop = 0.75, strata = cluster)
# 
# # Extract the training and test set
# df_train <- training(df_split)
# df_test  <- testing(df_split)
...
# # Train the model
# mod <- tree_spec %>% 
#   fit(formula = cluster ~ .,   data = df_train)
# 
# # Print the model
# mod
# 
...
#   mutate(true_class = df_test$cluster)
# 
# # Print the first lines of the result
# head(predictions_combined)
# 
# # The confusion matrix
# df_matrix <- conf_mat(data = predictions_combined,
#                             estimate = .pred_class,
#                             truth = true_class)
# 
# # Print the matrix
